...
export default function AppPrototype() {
  ...
  const handleAddActionToPlan = (planIndex: number) => { const newPlans = [...actionPlanForm.competencyPlans]; newPlans[planIndex].actions.push({ id: Date.now(), aspect: '', action: '', actionType: '', status: 'Sin Iniciar', deadline: '', observations: '' }); setActionPlanForm({ ...actionPlanForm, competencyPlans: newPlans }); };
                ...
                <div className="space-y-6 pb-24">
                    {competencies.map((comp) => (
                        <div key={comp.id} className="bg-white rounded-[8px] shadow-sm border border-[#E1DFDD] overflow-hidden">
                            ...
                            <div className="divide-y divide-[#E1DFDD]">{comp.aspectos.map((aspect) => (<div key={aspect.id} className="p-4 lg:p-6 hover:bg-[#FEFEFE] transition-colors"><p className="text-[#242424] font-medium mb-3">{aspect.question}</p><div className="w-full"><p className="text-xs text-[#605E5C] mb-2 uppercase font-semibold">{isReportMode ? 'Promedio Obtenido' : 'Frecuencia'}</p><RatingScale value={formResponses[aspect.id]} readOnly={isReadOnly} isAverageMode={isReportMode} onChange={(val: any) => handleResponseChange(aspect.id, val)}/></div></div>))}</div>
                        </div>
                    ))}
                                            ...
                                            <div className="space-y-4">
                                                {plan.actions.map((action: any, actionIndex: number) => {
                                                    ...
                                                    <div key={action.id} className="bg-[#F8F9FA] p-3 rounded-[4px] border border-[#E1DFDD] text-sm">
                                                        ...
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3"><div className="flex flex-col gap-1"><label className="text-[10px] text-[#605E5C] font-bold">Aspecto a Mejorar</label><select disabled={isActionPlanReadOnly} className={`w-full p-1.5 border border-[#E1DFDD] rounded text-xs ${isActionPlanReadOnly ? 'bg-[#F3F2F1]' : 'bg-white'}`} value={action.aspect} onChange={(e) => handleChangeActionInPlan(planIndex, actionIndex, 'aspect', e.target.value)}><option value="">Seleccione aspecto...</option>{availableAspects.map(asp => (<option key={asp.id} value={asp.question}>{asp.question}</option>))}</select></div><div className="flex flex-col gap-1"><label className="text-[10px] text-[#605E5C] font-bold">Acción Concreta</label><input type="text" disabled={isActionPlanReadOnly} className={`w-full p-1.5 border border-[#E1DFDD] rounded text-xs ${isActionPlanReadOnly ? 'bg-[#F3F2F1]' : 'bg-white'}`} placeholder="Descripción" value={action.action} onChange={(e) => handleChangeActionInPlan(planIndex, actionIndex, 'action', e.target.value)}/></div></div>
